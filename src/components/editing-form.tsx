...
import { type ModelType } from './generation-form';
...
export function EditingForm({
    ...
    const { t } = useTranslation('editingForm');
    ...
    const handleImageFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files) {
            ...
            if (totalFiles > maxImages) {
                            alert(t('form.sourceImages.maxImagesReached', { max: maxImages }));
    ...
    const handleMaskFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.type !== 'image/png') {
            alert(t('form.mask.invalidFileType'));
            event.target.value = '';
            return;
        }
        ...
        img.onerror = () => {
            alert(t('form.mask.failedToLoadMask'));
            URL.revokeObjectURL(objectUrl);
            event.target.value = '';
        };
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (imageFiles.length === 0) {
            alert(t('validation.selectAtLeastOneImage'));
            return;
        }
        if (editDrawnPoints.length > 0 && !editGeneratedMaskFile && !editIsMaskSaved) {
            alert(t('validation.saveMaskBeforeSubmit'));
            return;
        }
    ...
    const displayFileNames = (files: File[]) => {
        if (files.length === 0) return t('form.sourceImages.noFileSelected');
        if (files.length === 1) return t('form.sourceImages.fileSelected', { filename: files[0].name });
        return t('form.sourceImages.filesSelected', { count: files.length });
    };
            ...
            <CardHeader className='flex items-start justify-between border-b border-white/10 pb-4'>
                <div>
                    <div className='flex items-center'>
                        <CardTitle className='py-1 text-lg font-medium text-white'>{t('title')}</CardTitle>
                        {isPasswordRequiredByBackend && (
                            <Button
                                variant='ghost'
                                size='icon'
                                ...
                                aria-label={t('buttons.configurePassword')}>
                                {clientPasswordHash ? <Lock className='h-4 w-4' /> : <LockOpen className='h-4 w-4' />}
                            </Button>
                        )}
                    </div>
                    <CardDescription className='mt-1 text-white/60'>{t('description')}</CardDescription>
                </div>
            ...
            <form onSubmit={handleSubmit} className='flex h-full flex-1 flex-col overflow-hidden'>
                <CardContent className='flex-1 space-y-5 overflow-y-auto p-4'>
                    ...
                    <div className='space-y-2'>
                        <Label className='text-white'>{t('form.sourceImages.label')}</Label>
                        <Label
                    ...
                    <div className='space-y-3'>
                        <Label className='block text-white'>{t('form.mask.label')}</Label>
                        <Button
                            type='button'
                            variant='outline'
                            size='sm'
                            ...
                            {editShowMaskEditor
                                ? t('form.mask.closeMaskEditor')
                                : editGeneratedMaskFile
                                  ? t('form.mask.editSavedMask')
                                  : t('form.mask.createMask')}
                            {editIsMaskSaved && !editShowMaskEditor && (
                                <span className='ml-auto text-xs text-green-400'>({t('form.mask.savedIndicator')})</span>
                            )}
                            ...
                            <div className='space-y-3 rounded-md border border-white/20 bg-black p-3'>
                                ...
                                <div className='grid grid-cols-1 gap-4 pt-2'>
                                    <div className='space-y-2'>
                                        <Label htmlFor='brush-size-slider' className='text-sm text-white'>
                                            {t('form.mask.brushSize', { size: editBrushSize[0] })}
                                        </Label>
                                        <Slider
                                ...
                                <div className='flex items-center justify-between gap-2 pt-3'>
                                    <Button
                                        ...
                                        <UploadCloud className='mr-1.5 h-4 w-4' /> {t('form.mask.uploadMask')}
                                    </Button>
                                    <Input
                                        ref={maskInputRef}
                                        id='mask-file-input'
                                    ...
                                    <div className='flex gap-2'>
                                        <Button
                                            ...
                                            <Eraser className='mr-1.5 h-4 w-4' /> {t('form.mask.clearMask')}
                                        </Button>
                                        <Button
                                            type='button'
                                            variant='default'
                                            ...
                                            <Save className='mr-1.5 h-4 w-4' /> {t('form.mask.saveMask')}
                                        </Button>
                                    </div>
                                </div>
                                {editMaskPreviewUrl && (
                                    <div className='mt-3 border-t border-white/10 pt-3 text-center'>
                                        <Label className='mb-1.5 block text-sm text-white'>
                                            {t('form.mask.generatedMaskPreview')}
                                        </Label>
                                ...
                                {editIsMaskSaved && !editMaskPreviewUrl && (
                                    <p className='pt-1 text-center text-xs text-yellow-400'>
                                        {t('form.mask.generatingPreview')}
                                    </p>
                                )}
                                {editIsMaskSaved && editMaskPreviewUrl && (
                                    <p className='pt-1 text-center text-xs text-green-400'>{t('form.mask.maskSavedSuccess')}</p>
                                )}
                            </div>
                        )}
                        {!editShowMaskEditor && editGeneratedMaskFile && (
                            <p className='pt-1 text-xs text-green-400'>{t('form.mask.maskApplied', { filename: editGeneratedMaskFile.name })}</p>
                        )}
                    </div>

                    <div className='space-y-3'>
                        <Label className='block text-white'>{t('form.size.label')}</Label>
                        <RadioGroup
                            value={editSize}
                            ...
                            <RadioItemWithIcon
                                ...
                                label={t('form.size.options.landscape')}
                                Icon={RectangleHorizontal}
                            />
                            <RadioItemWithIcon
                                value='1024x1536'
                                id='edit-size-portrait'
                                label={t('form.size.options.portrait')}
                    ...
                    <div className='space-y-3'>
                        <Label className='block text-white'>{t('form.quality.label')}</Label>
                        <RadioGroup
                            value={editQuality}
                            onValueChange={(value) => setEditQuality(value as EditingFormData['quality'])}
                            disabled={isLoading}
                            className='flex flex-wrap gap-x-5 gap-y-3'>
                            <RadioItemWithIcon value='auto' id='edit-quality-auto' label={t('form.quality.options.auto')} Icon={Sparkles} />
                            <RadioItemWithIcon value='low' id='edit-quality-low' label={t('form.quality.options.low')} Icon={Tally1} />
                            <RadioItemWithIcon value='medium' id='edit-quality-medium' label={t('form.quality.options.medium')} Icon={Tally2} />
                            <RadioItemWithIcon value='high' id='edit-quality-high' label={t('form.quality.options.high')} Icon={Tally3} />
                        </RadioGroup>
                    </div>

                    <div className='space-y-2'>
                        <Label htmlFor='edit-n-slider' className='text-white'>
                            {t('form.numberOfImages.label', { count: editN[0] })}
                        </Label>
                        <Slider
                            id='edit-n-slider'
                            min={1}
                ...
                <CardFooter className='border-t border-white/10 p-4'>
                    <Button
                        ...
                        {isLoading ? t('buttons.editing') : t('buttons.edit')}
                    </Button>
                </CardFooter>
            </form>
        </Card>
